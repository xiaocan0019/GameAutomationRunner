...
$betterExe = Ask-Path "BetterGI 主程序路径，例如 D:\BetterGI\BetterGI.exe" (Get-ObjectValue $betterCurrent "Exe" $DefaultPaths.BetterGI.Exe) $false
$betterLogDefault = Get-ObjectValue $betterCurrent "LogDir" (Get-ChildDirFromExe $betterExe "log" $DefaultPaths.BetterGI.LogDir)
$betterLog = Ask-Path "BetterGI 日志目录，例如 D:\BetterGI\log" $betterLogDefault $true
$result.BetterGI = [ordered]@{ Exe = $betterExe; LogDir = $betterLog }

$marchCurrent = Get-TaskConfig $current "March7th"
$marchExe = Ask-Path "March7th Launcher 路径，例如 D:\March7thAssistant_full\March7th Launcher.exe" (Get-ObjectValue $marchCurrent "Exe" $DefaultPaths.March7th.Exe) $false
$marchAlt = Ask-Path "March7th Assistant 备用路径，例如 D:\March7thAssistant_full\March7th Assistant.exe" (Get-ObjectValue $marchCurrent "AlternateExe" $DefaultPaths.March7th.AlternateExe) $false
$marchLogDefault = Get-ObjectValue $marchCurrent "LogDir" (Get-ChildDirFromExe $marchExe "logs" $DefaultPaths.March7th.LogDir)
$marchLog = Ask-Path "March7th 日志目录，例如 D:\March7thAssistant_full\logs" $marchLogDefault $true
...
$maaExe = Ask-Path "MAA 主程序路径，例如 F:\MAA-v5.22.2-win-x64\MAA.exe" (Get-ObjectValue $maaCurrent "Exe" $DefaultPaths.MAA.Exe) $false
$maaDebugDefault = Get-ObjectValue $maaCurrent "DebugDir" (Get-ChildDirFromExe $maaExe "debug" $DefaultPaths.MAA.DebugDir)
$maaDebug = Ask-Path "MAA debug 日志目录，例如 F:\MAA-v5.22.2-win-x64\debug" $maaDebugDefault $true
